...
- ➀ $P$ is a shortest $(u_0, v)$-path,
- ➁ $u \in S$ and the $(u_0, u)$-section, $u_0 \cdots u$, is a shortest $(u_0, u)$-path, and
...
Equation {eq}`eq:9` motivates us to state the following proposition, which is the central idea of Dijkstra's algorithm.
...
:alt: A First Attempt to Find the Shortest Paths
:align: center
```
````
